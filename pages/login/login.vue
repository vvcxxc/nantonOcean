<template>
	<view class="login-page">
		...
		<uni-popup ref="popup" type="dialog">
			<uni-popup-dialog mode="input" title="请输入库台号" :duration="2000" :before-close="true" @close="close"
				placeholder="请输入数字" @confirm="confirm"></uni-popup-dialog>
		</uni-popup>
		<uni-popup ref="errorMsg" type="message">
			<uni-popup-message type="error" message="库台号请输入正整数！" :duration="5000"></uni-popup-message>
		</uni-popup>
	</view>
</template>
		...
		methods: {
			...
			confirm(value) {
				...
				if (re.test(value)) {
					...
					setTimeout(() => that.$refs.errorMsg.open());
				}
			},
			close() {
			...
			checkKuTai() {
				...
				uni.getStorage({
					...
					success: function(res) {
						if (!res.data) {
							setTimeout(() => that.$refs.popup.open())
						}
					},
					fail: function(res) {
						setTimeout(() => that.$refs.popup.open())
